...
def build_hit(raw: dict, media_type: str, base_dir: str, source_collection: str = "") -> dict:
    return {
        ...
        "customer_type": str(raw.get("customer_type") or "").strip(),
        ...
        docs.append(
            {
                ...
                "object": next((str(c.get("object") or "").strip() for c in chunks if str(c.get("object") or "").strip()), ""),
                "purpose": next((str(c.get("purpose") or "").strip() for c in chunks if str(c.get("purpose") or "").strip()), ""),
